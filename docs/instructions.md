...
[`ANC`](#anc-a-a-imm-c-bit-7)
[`AXS`](#axs-x-a-x-imm-no-borrow)
[`DCP`](#dcp-decrement-mem-then-compare-acc-to-mem)
[`ISC`](#isc-increment-mem-then-subtract-mem-from-acc-with-borrow)
[`LAX`](#lax-load-to-acc-and-x)
[`NOP`](#nop-nop-with-immediate-lasting-3-cycles)
[`SAX`](#sax-mem-a-x)
...
### `DCP` decrement mem, then compare Acc to mem
...
Acc|Implied|Imm|Mem|Mem,X|Mem,Y|(Mem,X)|(Mem),Y|(Mem)
:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:
|||`a?--mem`|`a?--mem,x`|`a?--mem,y`|`a?--(mem,x)`|`a?--(mem),y`|
...
### `ISC` increment mem, then subtract mem from Acc (with borrow)
...
```none
     addressing    assembler    opc  bytes  cyles
     --------------------------------------------
     zeropage      ISC arg      $E7    2     5
     zeropage,X    ISC arg,X    $F7    2     6
...
Acc|Implied|Imm|Mem|Mem,X|Mem,Y|(Mem,X)|(Mem),Y|(Mem)
:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:
|||`a=x=mem`||`a=x=mem,y`|`a=x=(mem,x)`|`a=x=(mem),y`|


```none
     addressing    assembler    opc  bytes  cyles
     --------------------------------------------
     zeropage      LAX arg      $A7    2     3
     zeropage,Y    LAX arg,Y    $B7    2     4
...
### `NOP` NOP with #immediate lasting 3 cycles

No operation.

Status flags: -

* `*<number>` with **even** `<number>` generates one NOP `#imm` and regular NOPs
